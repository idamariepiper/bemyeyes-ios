...
- (void)loginWithEmail:(NSString *)email userId:(NSString *)userId deviceToken:(NSString *)deviceToken success:(void (^)(BMEToken *token))success failure:(void (^)(NSError *error))failure;
